...
public class Portal extends Local implements IPortal {
  ...
  // Make method that allows a player to capture a portal with TeamType.Neutral, and if so the player needs to have atleast 25% of the maximum energy

public void capturePortal(Player player, int energy) {
  if (this.status == TeamType.NEUTRAL) {
    //Its missing the method that get the maximum energy of the player
    /*
    * This checks if the energy is less or equal to current energy of the player and energy is greater or equal to 25% of the maximum energy of the player
    */
    if ((energy <= player.getCurrentEnergy()) &&(energy>= player.getDefaultEnergy() * 0.25)) {
